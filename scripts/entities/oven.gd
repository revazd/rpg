...
func start_cooking(recipe: Dictionary, temp: int) -> bool:
	if recipe.is_empty():
		return false
	...
	for ingredient_id in recipe["required_ingredients"]:
		...
		if not Inventory.has_item(ingredient_id, qty):
			var item = ItemRegistry.get_item(ingredient_id)
			var item_name: String = item.item_name if item else str(ingredient_id)
			_notify("Il te manque : %s x%d" % [item_name, qty])
			return false

	# Retire ingrédients
	for ingredient_id in recipe["required_ingredients"]:
		Inventory.remove_item(ingredient_id, recipe["required_ingredients"][ingredient_id])
...
func _on_cooking_done() -> void:
	if not is_cooking or current_recipe.is_empty():
		return

	var temp_ok: bool = abs(chosen_temp - current_recipe["required_temp"]) <= current_recipe["temp_tolerance"]
	...
	Inventory.add_item(current_recipe["result_item"], 1)
...
func _trigger_burnt() -> void:
	...
	Inventory.add_item(current_recipe["burnt_item"], 1)
	current_recipe = {}
